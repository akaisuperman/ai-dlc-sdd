---
description: Review chéo — Dev A review spec của Dev B
---

# aiteam-review-spec

Review chéo spec của module do dev khác viết.
...
3. Thực hiện review theo skill `cross-review`:
...
- needs-revision → owner quay `aiteam-spec` sửa rồi submit review lại
- blocked (contract conflict) → quay `aiteam-kickoff` cập nhật contracts

## Next

- **approved** → owner chạy `aiteam-implement`
- **needs-revision** → owner chạy `aiteam-spec` sửa, rồi review lại
- **blocked** → escalate cho cả team, có thể cần `aiteam-kickoff`
